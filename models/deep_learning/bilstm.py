...
class BiLSTMRegressor(DLRegressor):
    """
    This is a class implementing the BiLSTM model for time series regression.
    ...
    def build_model(self, input_shape):
        ...
        if is_spline:
            ...
            x = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(int(dim1), return_sequences=True, kernel_regularizer=kernel_regularizer1,
        ...
        if float(dropout1) > 0:
            x = tf.keras.layers.Dropout(rate=float(dropout1))(x)

        x = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(int(dim2), return_sequences=False, kernel_regularizer=kernel_regularizer2,
        ...
        model = tf.keras.models.Model(inputs=input_layer, outputs=x, name='bilstm')

...
class Regressor:
    ...
    def fit(self,
        ...
        self.model.fit(x_train, y_train, sample_weight = density_weight[0])
    ...
    def predict(self, x: np.array):
        ...
        pickle.dump(self.model, open(self.output_directory + "finalized_model.sav", 'wb')) # save model
...
class SVRRegressor(Regressor):
    ...
    def cv(self,
        ...
        if hpo == "grid":
            ...
            search.fit(x_train, y_train, sample_weight=self.density_weight)
...
class RFRegressor(Regressor):
    ...
    def __init__(self,
        ...
        Inputs:
            output_directory: path to store results/models
            verbose: verbosity
            model_params: parameters for RF in dictionary format
        """
    ...
    def cv(self,
        ...
        best_param = None
        best_param_score = None
        print(self.density_weight.shape)
        if hpo == "grid":
            search_params = {
    ...
    def cv(self,
        ...
        if hpo == "grid":
            ...
            search = GridSearchCV(XGBRegressor(**params), search_params, n_jobs=1, cv=3,
                                  scoring="neg_mean_squared_error", verbose=True)
            search.fit(x_train, y_train, **fit_params, sample_weight=self.density_weight)
            best_param = search.best_params_
            best_param_score = search.best_score_
        elif hpo == "random":
            search_params = {
